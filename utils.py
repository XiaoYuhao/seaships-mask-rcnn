...
def readxml(path, labels_dict, device):
...
def drawRectangle(image, boxes):
    draw = ImageDraw.Draw(image)
    for box in boxes:
        draw.rectangle(box, outline=(255,0,0), width=4)

def drawMasks(image,mask):
    #draw = ImageDraw.Draw(image)
    #draw.bitmap((0,0), mask, fill = (0,255,0,200))
    #image.show()
    img = Image.new("RGBA", size=image.size, color=(0,255,0,0))
    draw = ImageDraw.Draw(img)
    draw.bitmap((0,0), mask, fill = (160,32,240,160))
    #img.show()
    #print("???")
