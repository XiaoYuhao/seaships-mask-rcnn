...
class MaskRCNN(FasterRCNN):
    ...
    def __init__(self, backbone, num_classes=None,
        ...
        if mask_roi_pool is None:
            mask_roi_pool = MultiScaleRoIAlign(                                                     #RoIAlign层固定输出尺寸为14*14，采样点2*2
        ...
        #self.roi_heads.weakly_supervision_train = self.weakly_supervision_train
...
class MaskRCNNHeads(nn.Sequential):
    def __init__(self, in_channels, layers, dilation):
        ...
        for layer_idx, layer_features in enumerate(layers, 1):                                      #增加这个是为了获取语义分割所需的特征？？
            d["mask_fcn{}".format(layer_idx)] = misc_nn_ops.Conv2d(                                 #全卷积层，不下采样，从上一层RoiAlign传来的尺寸是14*14
...
class MaskRCNNPredictor(nn.Sequential):
    def __init__(self, in_channels, dim_reduced, num_classes):
        super(MaskRCNNPredictor, self).__init__(OrderedDict([
            ("conv5_mask", misc_nn_ops.ConvTranspose2d(in_channels, dim_reduced, 2, 2, 0)),         #逆卷积层，上采样
            ("relu", nn.ReLU(inplace=True)),
            ("mask_fcn_logits", misc_nn_ops.Conv2d(dim_reduced, num_classes, 1, 1, 0)),             #输出通道为类别数，对应某个类别的mask，尺寸为7*7
        ]))
...
class BoxFeatureConverter(nn.Sequential):
